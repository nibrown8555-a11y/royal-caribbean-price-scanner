...
function parsePrice(text) {
  ...
  for(const p of [
    ...
    /\$\s*([0-9]{2,3}(?:\.[0-9]{2})?)\s*(?:USD)?[^\n]{0,180}(?:per person per day|pppd)/i,
    /from\s*\$\s*([0-9]{2,3}(?:\.[0-9]{2})?)/i
  ]) { const m=slice.match(p); if(m) return Number(m[1]); }
  return null;
}

async function frameDiagnostics(page) {
  const out=[];
  for(const f of page.frames()) {
    let title='', inputs=[];
    try { title=await f.title(); } catch {}
    try { inputs=await f.locator('input').evaluateAll(els=>els.map(e=>({type:e.type,name:e.name,id:e.id,placeholder:e.placeholder,autocomplete:e.autocomplete})).slice(0,20)); } catch {}
    out.push({url:f.url(),title,inputs});
  }
  console.log('LOGIN_FRAMES '+JSON.stringify(out));
  return out;
}
  ...
  for(const f of page.frames()) {
    for(const s of selectors) {
      try {
        ...
        if(await loc.count() && await loc.isVisible({timeout:800})) { await loc.fill(value); return {ok:true,frame:f}; }
      } catch {}
    }
  }
...
async function clickInFrames(page, selectors) {
  for(const f of page.frames()) {
    for(const s of selectors) {
      try {
        const loc=f.locator(s).first();
        if(await loc.count() && await loc.isVisible({timeout:800})) { await loc.click(); return true; }
      } catch {}
    }
  }
  return false;
...
async function ensureLoggedIn(page) {
  ...
  await frameDiagnostics(page);

  const email=await fillInFrames(page,[
    'input[type="email"]','input[autocomplete="email"]','input[autocomplete="username"]','input[name*="email" i]','input[id*="email" i]','input[placeholder*="email" i]','input[name*="user" i]'
  ],ROYAL_EMAIL);
  const pass=await fillInFrames(page,[
    'input[type="password"]','input[autocomplete="current-password"]','input[name*="password" i]','input[id*="password" i]','input[placeholder*="password" i]'
  ],ROYAL_PASSWORD);

  if(!email.ok || !pass.ok) return {ok:false,reason:'login_fields_not_found',text};
  ...
  text=(await page.locator('body').innerText().catch(()=>'' )).slice(0,50000);
  lower=text.toLowerCase();
  if(lower.includes('sign in') && !lower.includes('manage reservation')) return {ok:false,reason:'post_login_check_failed',text};
  return {ok:true};
}
...
async function scanOnce() {
  ...
  try {
    ...
    if(!auth.ok) {
      ...
      await pool.query(`insert into scans(ship,sail_date,package_name,page_url,status,raw_text) values($1,$2,$3,$4,$5,$6)`,[CRUISE_SHIP,SAIL_DATE,PACKAGE_NAME,url,status,String(text).slice(0,12000)]);
      await recordEvent(status,`Royal Caribbean login status: ${status}`,{url});
      if(status==='interactive_verification_required') await sendPushover('⚠️ Royal login needs verification','Royal Caribbean requested a verification step. Open Royal Caribbean and complete sign-in once, then we can refresh the scanner session.');
    ...
    if(price==null) { await recordEvent('parse_failed','Could not locate Deluxe Beverage Package price',{url}); result={ok:false,status,url}; return result; }
