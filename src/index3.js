...
const ROYAL_STORAGE_STATE_GZIP_B64 = process.env.ROYAL_STORAGE_STATE_GZIP_B64 || '';
...
function storageState() {
  if (!ROYAL_STORAGE_STATE_GZIP_B64) return null;
  const json = zlib.gunzipSync(Buffer.from(ROYAL_STORAGE_STATE_GZIP_B64,'base64')).toString('utf8');
  return JSON.parse(json);
}
function sessionSummary(){
  try { const s=storageState(); return {configured:!!s,cookieCount:s?.cookies?.length||0,originCount:s?.origins?.length||0,valid:true}; }
  catch { return {configured:true,cookieCount:0,originCount:0,valid:false}; }
}
...
function parsePrice(text){const i=text.toLowerCase().indexOf(PACKAGE_NAME.toLowerCase());const s=i>=0?text.slice(i,i+12000):text;for(const p of [/\$\s*([0-9]{2,3}(?:\.[0-9]{2})?)\s*(?:USD)?\s*(?:\/|per)\s*(?:person\s*)?(?:\/|per)?\s*day/i,/\$\s*([0-9]{2,3}(?:\.[0-9]{2})?)[^\n]{0,300}(?:per person per day|pppd)/i,/from\s*\$\s*([0-9]{2,3}(?:\.[0-9]{2})?)/i]){const m=s.match(p);if(m)return Number(m[1]);}return null;}
...
async function scanOnce(){
  ...
  try{
    ...
    const price=parsePrice(text); const promoText=text.match(/(?:up to\s+)?\d{1,2}%\s*off[^\n]*/i)?.[0]||null; const status=price==null?'parse_failed':'ok';
